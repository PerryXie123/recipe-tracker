...
export function useKitchens(userId?: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    const [kitchenResult, preferenceResult, memberResult, inviteResult] = await Promise.all([
      ...
      client.from("kitchen_members").select("kitchen_id,user_id,email,display_name,joined_at").order("joined_at"),
    ...
    const nextKitchens = (kitchenResult.data || []) as Kitchen[];
    ...
    setMembers((memberResult.data || []) as KitchenMember[]);
